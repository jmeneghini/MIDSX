...
std::vector<std::unique_ptr<VolumeTally>> initializeVolumeTallies() {
    ...
    tallies.emplace_back(std::make_unique<AACuboidVolumeTally>(
            Eigen::Vector3d(155, 0, 0),
            Eigen::Vector3d(175, 39.0, 39.0),
            volume_container));
...
//            39.0/2, 39.0/2, 180)));
    std::unique_ptr<Directionality> directionality = std::make_unique<RectangularIsotropicDirectionality>(
            RectangularIsotropicDirectionality(Eigen::Vector3d(180, 0, 0),
                                               Eigen::Vector3d(0, 39.0, 0),
                                               Eigen::Vector3d(0, 0, 39.0)));
    std::unique_ptr<SourceGeometry> geometry = std::make_unique<PointGeometry>(PointGeometry(Eigen::Vector3d(0, 39.0/2, 39.0/2)));
